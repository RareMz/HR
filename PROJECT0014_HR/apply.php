    ...
    if (!tableExists($conn, 'application_education')) {
        $conn->query("
            CREATE TABLE IF NOT EXISTS `application_education` (
              `id` int(11) NOT NULL AUTO_INCREMENT,
              `application_id` int(11) NOT NULL,
              `level` enum('secondary','vocational','university','preuni','college') NOT NULL,
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$already_applied) {
    ...
    $application_data = [
        'personal_info' => [
            ...
            // ข้อมูลทหาร (เพิ่มใหม่)
            'military_service' => $_POST['military_service'] ?? '',
            'military_service_when' => $_POST['military_service_when'] ?? '',
            'military_service_exempt_reason' => $_POST['military_service_exempt_reason'] ?? '',
            // ข้อมูลผู้ติดต่อฉุกเฉิน (เพิ่มใหม่)
            'emergency_contact_name' => $_POST['emergency_contact_name'] ?? '',
            'emergency_contact_address' => $_POST['emergency_contact_address'] ?? '',
            'emergency_contact_tel' => $_POST['emergency_contact_tel'] ?? '',
        ...
        'education' => [
            ...
            'preuni' => [  // เพิ่มเตรียมอุดมศึกษา
                'from' => $_POST['preuni_from'] ?? '',
                'to' => $_POST['preuni_to'] ?? '',
                'institute' => $_POST['preuni_institute'] ?? '',
                'degree' => $_POST['preuni_degree'] ?? '',
                'major' => $_POST['preuni_major'] ?? ''
            ],
            'vocational' => [
            ...
            'college' => [  // เพิ่มวิทยาลัย
                'from' => $_POST['college_from'] ?? '',
                'to' => $_POST['college_to'] ?? '',
                'institute' => $_POST['college_institute'] ?? '',
                'degree' => $_POST['college_degree'] ?? '',
                'major' => $_POST['college_major'] ?? ''
            ],
            'university' => [
        ...
        if (count($missing_fields) > 0) {
            ...
            if ($stmt->execute()) {
                ...
                $stmt = $conn->prepare("
                    ...
                    religion, id_card, issued_at, issued_date, 
                    military_service, military_service_when, military_service_exempt_reason,
                    emergency_contact_name, emergency_contact_address, emergency_contact_tel, emergency_contact_relationship,
                    photo_path) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                ...
                $stmt->bind_param(
                    ...
                    $issued_date,
                    $application_data['personal_info']['military_service'],
                    $application_data['personal_info']['military_service_when'],
                    $application_data['personal_info']['military_service_exempt_reason'],
                    $application_data['personal_info']['emergency_contact_name'],
                    $application_data['personal_info']['emergency_contact_address'],
                    $application_data['personal_info']['emergency_contact_tel'],
                    $application_data['personal_info']['emergency_contact_relationship'],
                    $photo_path
                );
                
                $stmt->execute();
                
                // บันทึกข้อมูลการศึกษาแต่ละระดับ
                $education_levels = ['secondary', 'preuni', 'vocational', 'college', 'university'];
                
                foreach ($education_levels as $level) {
                    if (!empty($application_data['education'][$level]['institute'])) {
                        try {
                            $stmt = $conn->prepare("
                            ...
                            $stmt->bind_param(
                                ...
                                $application_id,
                                $level,
                                $application_data['education'][$level]['from'],
                                $application_data['education'][$level]['to'],
                                $application_data['education'][$level]['institute'],
                                $application_data['education'][$level]['degree'],
                                $application_data['education'][$level]['major']
                            );
                            
                            $stmt->execute();
                        } catch (Exception $e) {
                            error_log("Error saving {$level} education: " . $e->getMessage());
                        }
                    }
                }
...
<body>
            ...
            <?php if (!$submit_success): ?>
                <div class="application-form">
                    ...
                    <form method="post" enctype="multipart/form-data" id="applicationForm">
                        ...
                        <!-- ข้อมูลตำแหน่ง (เพิ่มจาก userrecruitmentform.html) -->
                        <div class="form-section" id="positionSection">
                            <h3 class="form-title">
                                ข้อมูลตำแหน่ง / Position Information
                                <span class="section-indicator">ส่วนที่ 1/5</span>
                            </h3>
                            
                            <div class="form-row">
                                <div class="form-group">
                                    <label>ตำแหน่งที่สมัคร / Position Applied</label>
                                    <input type="text" name="position" value="<?php echo htmlspecialchars($position['title']); ?>" readonly>
                                </div>
                                <div class="form-group">
                                    <label>เงินเดือนที่ต้องการ / Desired Salary</label>
                                    <input type="number" name="desired_salary" value="<?php echo $draft_data ? htmlspecialchars($draft_data['desired_salary'] ?? '') : ''; ?>">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section" id="personalSection">
                            <h3 class="form-title">
                                ...
                                <span class="section-indicator">ส่วนที่ 2/5</span>
                            </h3>
                        ...
                        <div class="form-section" id="educationSection">
                            <h3 class="form-title">
                                ...
                                <span class="section-indicator">ส่วนที่ 3/5</span>
                            </h3>
                            ...
                            <table class="education-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>เตรียมอุดมศึกษา<br>Pre-University</td>
                                        <td>
                                            <div class="period-inputs">
                                                <input type="text" name="preuni_from" placeholder="From" value="<?php echo $draft_data && isset($draft_data['education']['preuni']) ? htmlspecialchars($draft_data['education']['preuni']['from']) : ''; ?>">
                                                <span>/</span>
                                                <input type="text" name="preuni_to" placeholder="To" value="<?php echo $draft_data && isset($draft_data['education']['preuni']) ? htmlspecialchars($draft_data['education']['preuni']['to']) : ''; ?>">
                                            </div>
                                        </td>
                                        <td><input type="text" name="preuni_institute" value="<?php echo $draft_data && isset($draft_data['education']['preuni']) ? htmlspecialchars($draft_data['education']['preuni']['institute']) : ''; ?>"></td>
                                        <td><input type="text" name="preuni_degree" value="<?php echo $draft_data && isset($draft_data['education']['preuni']) ? htmlspecialchars($draft_data['education']['preuni']['degree']) : ''; ?>"></td>
                                        <td><input type="text" name="preuni_major" value="<?php echo $draft_data && isset($draft_data['education']['preuni']) ? htmlspecialchars($draft_data['education']['preuni']['major']) : ''; ?>"></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td>วิทยาลัย Technical/<br>Institute/College</td>
                                        <td>
                                            <div class="period-inputs">
                                                <input type="text" name="college_from" placeholder="From" value="<?php echo $draft_data && isset($draft_data['education']['college']) ? htmlspecialchars($draft_data['education']['college']['from']) : ''; ?>">
                                                <span>/</span>
                                                <input type="text" name="college_to" placeholder="To" value="<?php echo $draft_data && isset($draft_data['education']['college']) ? htmlspecialchars($draft_data['education']['college']['to']) : ''; ?>">
                                            </div>
                                        </td>
                                        <td><input type="text" name="college_institute" value="<?php echo $draft_data && isset($draft_data['education']['college']) ? htmlspecialchars($draft_data['education']['college']['institute']) : ''; ?>"></td>
                                        <td><input type="text" name="college_degree" value="<?php echo $draft_data && isset($draft_data['education']['college']) ? htmlspecialchars($draft_data['education']['college']['degree']) : ''; ?>"></td>
                                        <td><input type="text" name="college_major" value="<?php echo $draft_data && isset($draft_data['education']['college']) ? htmlspecialchars($draft_data['education']['college']['major']) : ''; ?>"></td>
                                    </tr>
                                    <tr>
                        ...
                        <!-- การรับราชการทหาร (เพิ่มส่วนนี้จาก userrecruitmentform.html) -->
                        <div class="form-section" id="militarySection">
                            <h3 class="form-title">
                                การรับราชการทหาร / Military Service
                                <span class="section-indicator">ส่วนที่ 4/5</span>
                            </h3>
                            
                            <div class="form-group">
                                <label>ท่านผ่านการเกณฑ์ทหารมาแล้วหรือยัง / Have you completed your national service?</label>
                                <div class="radio-group">
                                    <label class="radio-option">
                                        <input type="radio" name="military_service" value="yes" <?php echo $draft_data && isset($draft_data['personal_info']['military_service']) && $draft_data['personal_info']['military_service'] == 'yes' ? 'checked' : ''; ?>>
                                        เกณฑ์แล้ว / Yes
                                    </label>
                                    <label class="radio-option">
                                        <input type="radio" name="military_service" value="no" <?php echo $draft_data && isset($draft_data['personal_info']['military_service']) && $draft_data['personal_info']['military_service'] == 'no' ? 'checked' : ''; ?>>
                                        ยังไม่ได้เกณฑ์ / No
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    <label>ถ้าต้องเข้าเกณฑ์เมื่อ / If so, Specify when</label>
                                    <input type="text" name="military_service_when" value="<?php echo $draft_data && isset($draft_data['personal_info']['military_service_when']) ? htmlspecialchars($draft_data['personal_info']['military_service_when']) : ''; ?>">
                                </div>
                                <div class="form-group">
                                    <label>ได้รับการยกเว้นเพราะ / If exempted, specify reason</label>
                                    <input type="text" name="military_service_exempt_reason" value="<?php echo $draft_data && isset($draft_data['personal_info']['military_service_exempt_reason']) ? htmlspecialchars($draft_data['personal_info']['military_service_exempt_reason']) : ''; ?>">
                                </div>
                            </div>
                        </div>

                        <!-- บุคคลที่ติดต่อได้ฉุกเฉิน (เพิ่มส่วนนี้จาก userrecruitmentform.html) -->
                        <div class="form-section" id="emergencyContactSection">
                            <h3 class="form-title">
                                กรณีเร่งด่วน บุคคลที่สามารถติดต่อได้ / In case of emergency, please notify
                                <span class="section-indicator">ส่วนที่ 5/5</span>
                            </h3>
                            
                            <div class="form-row">
                                <div class="form-group">
                                    <label>ชื่อ / Name</label>
                                    <input type="text" name="emergency_contact_name" value="<?php echo $draft_data && isset($draft_data['personal_info']['emergency_contact_name']) ? htmlspecialchars($draft_data['personal_info']['emergency_contact_name']) : ''; ?>">
                                </div>
                                <div class="form-group">
                                    <label>ที่อยู่ / Address</label>
                                    <input type="text" name="emergency_contact_address" value="<?php echo $draft_data && isset($draft_data['personal_info']['emergency_contact_address']) ? htmlspecialchars($draft_data['personal_info']['emergency_contact_address']) : ''; ?>">
                                </div>
                            </div>

                            <div class="form-row">
                                <div class="form-group">
                                    <label>โทรศัพท์ / Tel No.</label>
                                    <input type="tel" name="emergency_contact_tel" value="<?php echo $draft_data && isset($draft_data['personal_info']['emergency_contact_tel']) ? htmlspecialchars($draft_data['personal_info']['emergency_contact_tel']) : ''; ?>">
                                </div>
                                <div class="form-group">
                                    <label>ความสัมพันธ์ / Relationship</label>
                                    <input type="text" name="emergency_contact_relationship" value="<?php echo $draft_data && isset($draft_data['personal_info']['emergency_contact_relationship']) ? htmlspecialchars($draft_data['personal_info']['emergency_contact_relationship']) : ''; ?>">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-section" id="documentsSection">
                            ...
                            <div class="form-group">
                                <label class="required">อัปโหลด (PDF เท่านั้น)</label>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // การปรับขนาดหน้าจอ - เหมือนนักวิ่งที่ปรับจังหวะตามภูมิประเทศ
            ...
            // จัดการการอัพโหลดรูปภาพ - เก็บภาพความทรงจำของนักวิ่ง
            ...
            // คำนวณอายุอัตโนมัติจากวันเกิด - เหมือนระบบวัด VO2 Max ที่คำนวณประสิทธิภาพของนักวิ่ง
            ...
            // ตรวจสอบเลขบัตรประชาชน - เหมือนการตรวจสอบ race bib ของนักวิ่ง
            ...
            // จัดการกับ progress bar - แสดงความคืบหน้าเหมือนแอปติดตามการวิ่งของคุณ
            function updateProgressBar() {
                ...
                // นับช่องที่กรอกแล้ว - เหมือนนับก้าวในการวิ่ง
                ...
                // ตรวจสอบไฟล์ที่อัพโหลด - เหมือนตรวจอุปกรณ์วิ่งให้พร้อม
                const resumeInput = document.getElementById('resume');
                const photoInput = document.getElementById('photoInput');
                const resumeUploaded = resumeInput && 
                    ((resumeInput.files && resumeInput.files.length > 0) || 
                    document.querySelector('p:contains("เอกสารที่อัพโหลดไว้")'));
                const photoUploaded = photoPreview && 
                    (photoPreview.style.display === 'block');
                
                if (resumeUploaded) filledCount++;
                if (photoUploaded) filledCount++;
                
                totalFields += 2; // เพิ่มสำหรับไฟล์ resume และ photo
                
                // คำนวณเปอร์เซ็นต์ความคืบหน้า - เหมือนเปอร์เซ็นต์ของเส้นทางวิ่งที่ผ่านมา
                ...
                if (progressBar && progressPercentage) {
                    ...
                    // ปรับสีของแถบความคืบหน้าตามค่า - เหมือนการปรับโซน HR ในการวิ่ง
                    if (progress < 30) {
                        progressBar.style.background = 'linear-gradient(90deg, #f56565, #ed8936)';
                    } else if (progress < 70) {
                        progressBar.style.background = 'linear-gradient(90deg, #ed8936, #ecc94b)';
                    } else {
            ...
            // เรียกใช้ฟังก์ชันตอนโหลดเพจ - เหมือนวอร์มอัพก่อนวิ่ง
            updateProgressBar();
            
            // ติดตามการเปลี่ยนแปลงข้อมูล - เหมือน GPS ติดตามการวิ่งแบบเรียลไทม์
            ...
            // เพิ่มการยืนยันก่อนออกจากหน้าถ้ามีการแก้ไขฟอร์ม - เหมือนแอปเตือนเมื่อคุณหยุดวิ่งกลางทาง
            ...
            window.addEventListener('beforeunload', function(e) {
                if (formChanged) {
                    const confirmationMessage = 'คุณมีข้อมูลที่ยังไม่ได้บันทึก ต้องการออกจากหน้านี้จริงหรือไม่?';
                    (e || window.event).returnValue = confirmationMessage;
                    return confirmationMessage;
                }
            });
            
            // ยกเลิกการเตือนเมื่อกดปุ่มบันทึกหรือส่ง
            document.querySelectorAll('button[name="save_as_draft"], button[name="submit_application"]').forEach(button => {
