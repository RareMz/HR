...
<body>
    ...
    <!-- Notification แจ้งผลการส่งอีเมล -->
    <div id="emailSentNotification" class="notification success">
        <div class="notification-icon">✓</div>
        <div class="notification-content">
            <div class="notification-title">ส่งอีเมลสำเร็จ</div>
            <div class="notification-message">กรุณาตรวจสอบอีเมลของคุณเพื่อรีเซ็ตรหัสผ่าน</div>
        </div>
        <button class="notification-close" onclick="hideEmailSentNotification()">✕</button>
    </div>

    <!-- ระบบแสดงลิงก์รีเซ็ตรหัสผ่าน (สำหรับการทดสอบเท่านั้น) -->
    <div id="resetLinkDemo" class="notification success" style="display: none; border-left: 4px solid #805ad5;">
        <div class="notification-icon" style="background: #e9d8fd; color: #805ad5;">🔗</div>
        <div class="notification-content">
            <div class="notification-title">ลิงก์สำหรับทดสอบ</div>
            <div class="notification-message">
                <a href="#" id="reset-link-url" target="_blank" style="color: #805ad5; text-decoration: underline;">คลิกที่นี่เพื่อรีเซ็ตรหัสผ่าน</a>
            </div>
        </div>
        <button class="notification-close" onclick="hideResetLinkDemo()">✕</button>
    </div>
    ...
    <div class="modal-overlay" id="modalOverlay">
        <div class="modal-container">
            ...
            <form id="login-form" class="form-section active" action="login.php" method="post">
                ...
                <a href="#" class="forgot-link" id="forgotPasswordLink">ลืมรหัสผ่าน?</a>
    ...
    <div class="modal-overlay" id="forgotPasswordModal">
        <div class="modal-container">
            <button class="close-btn" title="ปิด" id="closeForgotModalBtn">&times;</button>
            
            <h1 style="font-size: 1.8rem; margin-bottom: 1.5rem; text-align: center;">ลืมรหัสผ่าน</h1>
            <p style="text-align: center; margin-bottom: 2rem;">กรุณากรอกอีเมลที่ใช้ลงทะเบียน เราจะส่งลิงก์สำหรับรีเซ็ตรหัสผ่านไปให้คุณ</p>
            
            <!-- ข้อความแจ้งเตือน -->
            <div id="forgot-error-message" class="message error" style="display: none; padding: 1rem; border-radius: 10px; margin-bottom: 1.5rem; background-color: #fed7d7; color: #e53e3e;">
                <p id="forgot-error-text"></p>
            </div>
            
            <form id="forgot-form" action="forgot-password.php" method="post">
                <div class="form-group">
                    <input type="email" id="forgot-email" name="email" placeholder=" " required>
                    <label for="forgot-email">อีเมล</label>
                </div>
                <button type="submit" class="submit-btn">ส่งลิงก์รีเซ็ตรหัสผ่าน</button>
            </form>
            
            <p style="text-align: center; margin-top: 1.5rem;">
                <a href="#" onclick="showLoginModal(); return false;" class="forgot-link" style="margin: 0;">กลับไปหน้าเข้าสู่ระบบ</a>
            </p>
        </div>
    </div>
    ...
    <script>
        ...
        // เพิ่ม event สำหรับลิงก์ "ลืมรหัสผ่าน" ในฟอร์ม login
        document.getElementById('forgotPasswordLink')?.addEventListener('click', function(e) {
            e.preventDefault();
            // ซ่อน modal login และแสดง modal ลืมรหัสผ่าน
            hideLoginModal();
            showForgotPasswordModal();
        });

        // เพิ่มตัวแปรสำหรับ modal ลืมรหัสผ่าน
        const forgotPasswordModal = document.getElementById('forgotPasswordModal');
        const closeForgotModalBtn = document.getElementById('closeForgotModalBtn');
        const emailSentNotification = document.getElementById('emailSentNotification');
        const resetLinkDemo = document.getElementById('resetLinkDemo');

        // แสดง modal ลืมรหัสผ่าน
        function showForgotPasswordModal() {
            forgotPasswordModal.classList.add('active');
            document.body.style.overflow = 'hidden';
            // ซ่อน modal อื่นๆ ถ้ากำลังแสดงอยู่
            modalOverlay.classList.remove('active');
            
            // ซ่อนเมนูมือถือถ้ากำลังแสดงอยู่
            if (navMenu) {
                navMenu.classList.remove('active');
            }
            if (menuBtn) {
        ...
        // ซ่อน modal ลืมรหัสผ่าน
        function hideForgotPasswordModal() {
            forgotPasswordModal.classList.remove('active');
            document.body.style.overflow = '';
        }

        // แสดงข้อความแจ้งเตือนส่งอีเมลสำเร็จ
        function showEmailSentNotification() {
            hideForgotPasswordModal();
            emailSentNotification.classList.add('show');
            
            setTimeout(() => {
                hideEmailSentNotification();
            }, 7000);
            
            // สำหรับการทดสอบเท่านั้น: แสดงลิงก์สำหรับรีเซ็ตรหัสผ่าน
            if (typeof resetLinkUrl !== 'undefined' && resetLinkUrl) {
                document.getElementById('reset-link-url').href = resetLinkUrl;
                setTimeout(() => {
                    resetLinkDemo.classList.add('show');
                }, 1000);
            }
        }

        // ซ่อนข้อความแจ้งเตือนส่งอีเมลสำเร็จ
        function hideEmailSentNotification() {
            emailSentNotification.classList.remove('show');
        }

        // ซ่อนลิงก์ทดสอบ
        function hideResetLinkDemo() {
            resetLinkDemo.classList.remove('show');
        }

        // แสดงข้อความแจ้งเตือนในฟอร์มลืมรหัสผ่าน
        ...
        // ซ่อนข้อความแจ้งเตือนในฟอร์มลืมรหัสผ่าน
        ...
        // เพิ่ม event listener สำหรับการคลิกที่พื้นหลัง modal
        ...
        // เพิ่ม event listener สำหรับการส่งฟอร์มลืมรหัสผ่าน
        document.getElementById('forgot-form')?.addEventListener('submit', function(e) {
            const email = document.getElementById('forgot-email').value.trim();
            
            if (!email) {
                e.preventDefault();
                showForgotError('กรุณากรอกอีเมล');
                return;
            }
            ...
            if (!emailPattern.test(email)) {
                e.preventDefault();
                showForgotError('กรุณากรอกอีเมลให้ถูกต้อง');
                return;
            }
        ...
        // เช็ค URL parameters สำหรับแสดงข้อความแจ้งเตือน
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // เพิ่มโค้ดเช็ค URL parameters สำหรับลืมรหัสผ่าน
            const urlParams = new URLSearchParams(window.location.search);
            
            if (urlParams.has('showForgot')) {
                showForgotPasswordModal();
                ...
                if (urlParams.has('error')) {
                    ...
                    if (error === 'empty_email') {
                        ...
                        showForgotError('ไม่พบอีเมลนี้ในระบบ');
                    } else if (error === 'system') {
                        showForgotError('เกิดข้อผิดพลาดในระบบ กรุณาลองใหม่อีกครั้ง');
                    }
                }
            }
            
            // ตรวจสอบการส่งอีเมลสำเร็จ
            if (urlParams.has('email_sent')) {
                showEmailSentNotification();
                
                // ตรวจสอบว่ามีข้อมูล reset_url ใน session หรือไม่ (สำหรับการทดสอบ)
                <?php if (isset($_SESSION['reset_url'])): ?>
                const resetLinkUrl = "<?php echo $_SESSION['reset_url']; ?>";
                // แสดงลิงก์รีเซ็ตรหัสผ่านสำหรับทดสอบ
                document.getElementById('reset-link-url').href = resetLinkUrl;
                setTimeout(() => {
                    resetLinkDemo.classList.add('show');
                }, 1000);
            ...
            // ตรวจสอบข้อผิดพลาดของ token
            if (urlParams.has('error') && urlParams.get('error') === 'invalid_token') {
                const notification = document.getElementById('loginNotification');
                document.querySelector('#loginNotification .notification-title').textContent = 'เกิดข้อผิดพลาด';
                document.querySelector('#loginNotification .notification-message').textContent = 'โทเค็นไม่ถูกต้องหรือหมดอายุแล้ว';
                notification.classList.add('show');
