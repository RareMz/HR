...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_position'])) {
    ...
    $salary_min = isset($_POST['salary_min']) && $_POST['salary_min'] !== '' ? intval($_POST['salary_min']) : null;
    $salary_max = isset($_POST['salary_max']) && $_POST['salary_max'] !== '' ? intval($_POST['salary_max']) : null;
    $status = trim($_POST['status']);
    
    // Validation
    if (empty($title) || strlen($title) > 100) {
        $_SESSION['error'] = "กรุณาระบุชื่อตำแหน่งให้ถูกต้อง (ไม่เกิน 100 ตัวอักษร)";
    ...
    $stmt->bind_param("ssssssiiis", $title, $department, $description, $requirements, $type, $location, $salary_min, $salary_max, $status, $_SESSION['user_id']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_position'])) {
    ...
    $requirements = trim($_POST['edit_requirements']);
    $type = trim($_POST['edit_type']);
    $location = trim($_POST['edit_location']);
    $salary_min = isset($_POST['edit_salary_min']) && $_POST['edit_salary_min'] !== '' ? intval($_POST['edit_salary_min']) : null;
    $salary_max = isset($_POST['edit_salary_max']) && $_POST['edit_salary_max'] !== '' ? intval($_POST['edit_salary_max']) : null;
    ...
    $stmt->bind_param("ssssssiisii", $title, $department, $description, $requirements, $type, $location, $salary_min, $salary_max, $status, $_SESSION['user_id'], $position_id);
